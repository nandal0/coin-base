...
const Transfer = ({ setAction, twTokens, selectedToken, walletAddress }) => {
  ...
  useEffect(() => {
    ...
    // const { contract } = useContract("0x49d454d8109a097c2de5D34965d81A5A31a055aD", "token")
  ...
  const sendCrypto = async () => {
    ...
    if (activeTwToken && amount && recipient) {
      ...
      const balance = await activeTwToken.balance();
console.log(balance);
      ...
      const data = [
        {
          address:  recipient, 
          amount:amount.toString().concat('000000000000000000')
        }
      ]
      
      console.log(data)
      const result = await activeTwToken.transferBatch(data);
